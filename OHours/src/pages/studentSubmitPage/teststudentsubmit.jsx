...
import { mockRoomInfo, mockQueueLength, mockSubmitQuestion } from './mockData';
...
function StudentSubmitForm() {
  ...
  useEffect(() => {
    const fetchRoomInfo = async () => {
      if (USE_MOCK_DATA) {
        ...
        setQueueLength(mockQueueLength);
      } else {
        try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let docRef;
      if (USE_MOCK_DATA) {
        docRef = await mockSubmitQuestion(name, question);
      } else {
        const queueRef = collection(db, 'rooms', roomCode, 'queue');
        docRef = await addDoc(queueRef, {
          name,
          question,
